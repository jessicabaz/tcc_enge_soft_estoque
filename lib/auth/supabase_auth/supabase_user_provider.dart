...
class InventorySystemSupabaseUser extends BaseAuthUser {
  InventorySystemSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> inventorySystemSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = InventorySystemSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
